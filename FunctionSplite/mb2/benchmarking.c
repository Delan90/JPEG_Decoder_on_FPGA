...
{
  int i;
  unsigned char *cmem_in0 = (unsigned char*) (mb2_cmemin0_BASEADDR);
  unsigned char *cmem_in1 = (unsigned char*) (mb2_cmemin1_BASEADDR);
  unsigned char *cmem_in2 = (unsigned char*) (mb2_cmemin2_BASEADDR);
  unsigned char *cmem_in3 = (unsigned char*) (mb2_cmemin3_BASEADDR);
  
  unsigned char *cmem_out0 = (unsigned char*) (mb2_cmemout0_BASEADDR);
  unsigned char *cmem_out1 = (unsigned char*) (mb2_cmemout1_BASEADDR);
  ...
  {
    cmem_out0[i] = PATTERN2;
	cmem_out1[i] = PATTERN2;
  }
...
int bench_mark_measure_cmem_usage()
{
  ...
  for( i = 0 ; i < CMEM_IN_SIZE; i++)
  {
    comm_mem_size -= (cmem_in0[i] != PATTERN2) ? 1 : 0;
	comm_mem_size -= (cmem_in1[i] != PATTERN2) ? 1 : 0;
	comm_mem_size -= (cmem_in2[i] != PATTERN2) ? 1 : 0;
	comm_mem_size -= (cmem_in3[i] != PATTERN2) ? 1 : 0;
  }
    
  for( i = 0 ; i < CMEM_OUT_SIZE; i++)
  {
    comm_mem_size -= (cmem_out0[i] != PATTERN2) ? 1 : 0;
	comm_mem_size -= (cmem_out1[i] != PATTERN2) ? 1 : 0;
  }
