...
#else
unsigned long get_bits(unsigned char * fi, int number)
#endif
{
	int i, newbit;
...
#else
unsigned char get_one_bit(unsigned char * fi)
#endif
{
    int newbit;
    unsigned char aux, wwindow;
...
#else
unsigned int get_size(unsigned char * fi)
#endif
...
#else
void skip_segment(unsigned char * fi)
#endif
...
#else
unsigned int get_next_MK(unsigned char * fi)
#endif
{
	unsigned int c;
	int ffmet = 0;
...
#ifdef FILE_IO
    while ((c = fgetc(fi)) != (unsigned int)(0X0A)/*EOF*/) 
#else
	while ((c = FGETC(fi)) != (unsigned int)(0X0A)/*EOF*/) 
#endif
    {
...
#else
int load_quant_tables(unsigned char * fi)
#endif
{
	char aux;
...
#else
        MY_MK_MALLOC(QTable[id],PBlock,1); 
...
int init_MCU(void)
{
    ...
    volatile int* Core3DataPointer = (int*)mb3_cmemin1_pt_REMOTEADDR;
    volatile int* Core1DataMemory  = (int*)mb1_cmemout1_BASEADDR;
...
#else
            MY_MK_MALLOC(MCU_buff[k],PBlock,1); 
#endif
            if (MCU_buff[k] == NULL) {
#ifdef FILE_IO
    ...
    *((int*)(Core1DataMemory + 0)) = 1;
    *((int*)(Core1DataMemory + 1)) = MCU_sx;
    *((int*)(Core1DataMemory + 2)) = MCU_sy;
    *((int*)(Core1DataMemory + 3)) = mx_size;
    *((int*)(Core1DataMemory + 4)) = my_size;
    *((int*)(Core1DataMemory + 5)) = k;
    *((int*)(Core1DataMemory + 6)) = 0xFFFFFFFF;
    *((int*)(Core1DataMemory + 7)) = 0xFFFFFFFF;
    *((int*)(Core1DataMemory + 8)) = 0xFFFFFFFF;
    *((int*)(Core1DataMemory + 9)) = 1;

    for(i=0;i<n_comp;i++)
    {
        *((int*)(Core1DataMemory + 6 + i)) = comp[i].VDIV << 24 | comp[i].HDIV << 16 | comp[i].HS << 8 | comp[i].IDX;
    }

    DMA_SEND_BLOCKING((void *) Core3DataPointer,(int *) Core1DataMemory,10,(void *) mb1_dma1_BASEADDR,DMA_flag);
...
#else
int process_MCU(unsigned char *fi)
#endif
{
#if 0
    unsigned int i,j;
    ...
    if (n_comp > 1)
    {
        ...
        for(k=0;k<6;k++)
            for(l=0;l<64;l++)
                mk_mon_debug_info(MCU_buff[k]->linear[l]);
#endif
