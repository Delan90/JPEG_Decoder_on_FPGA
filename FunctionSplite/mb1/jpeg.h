...
/* Input file parsing DMA related constants */
#define INPUT_READ_SIZE_INT   128
#define INPUT_READ_SIZE       (INPUT_READ_SIZE_INT << 2) 
#define INPUT_READ_SIZE_BITS  (INPUT_READ_SIZE << 3)
#define NUM_OF_BLOCKS         4
#define NUM_OF_INIT_BUFF_LOAD 4
#define TOTAL_BLOCK_SIZE      (((INPUT_READ_SIZE) * NUM_OF_BLOCKS) - 1)
...
extern volatile fifo_t* producer; 
extern volatile fifo_t* consumer; 
extern volatile int* producer_data; 
extern volatile int* consumer_data; 

extern volatile int* fifo_sync;
extern volatile int* fifo_sync_data;
...
#define CHANGE_BUFFER(_var) ((_var+1) & (NUM_OF_BLOCKS - 1));
...
#else
void aborted_stream(unsigned char *fi);
...
#else
void unpack_block(unsigned char* fi, FBlock * T, int comp);
#endif
