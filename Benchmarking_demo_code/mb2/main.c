...
int bench_mark_measure_cmem_usage()
{
  ...
  for( i = 0 ; i < CMEM_IN_SIZE; i++)
  {
    comm_mem_size -= (cmem_in0[i] != PATTERN) ? 1 : 0;
	comm_mem_size -= (cmem_in1[i] != PATTERN) ? 1 : 0;
	comm_mem_size -= (cmem_in2[i] != PATTERN) ? 1 : 0;
	comm_mem_size -= (cmem_in3[i] != PATTERN) ? 1 : 0;
  }
    
  for( i = 0 ; i < CMEM_OUT_SIZE; i++)
  {
    comm_mem_size -= (cmem_out0[i] != PATTERN) ? 1 : 0;
	comm_mem_size -= (cmem_out1[i] != PATTERN) ? 1 : 0;
  }
