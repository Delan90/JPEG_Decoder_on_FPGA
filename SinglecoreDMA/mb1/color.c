...
void color_conversion(long ddr_offset)
{
	int i, j;
...
#if 1
         *(int*)(mb1_cmemout1_BASEADDR) = 
                          ((Saturate(r) << 16) & 0X00FF0000) 
                          | ((Saturate(g) << 8) & 0X0000FF00) 
                          | (Saturate(b) & 0X000000FF);
#endif
         hw_dma_send_addr((void*)(FrameBuffer + ddr_offset + i * 1024 + j), ((int*)(mb1_cmemout1_BASEADDR)), 1, (void*)mb1_dma1_BASEADDR);
#if 0
         mk_mon_debug_info(i); 
         mk_mon_debug_info(j); 
         mk_mon_debug_info(ddr_offset + i * 1024 + j); 
         mk_mon_debug_info(*(FrameBuffer + ddr_offset + i * 1024 + j)); 
#endif /* DEBUG */
