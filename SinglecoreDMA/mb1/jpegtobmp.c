...
#else 
#if 1
    hw_dma_receive_addr((int*)(mb1_cmemout0_BASEADDR), (volatile unsigned int *)(shared_pt_REMOTEADDR+1024*1024*4),
                        0xEF, (void*)mb1_dma0_BASEADDR);
    
    fi = (volatile unsigned int *)(mb1_cmemout0_BASEADDR);
#else
...
#else
                FrameBuffer = (volatile unsigned int *) shared_pt_REMOTEADDR;
#endif		
...
void clear_frame_buffer()
{
	...
	for (i = 0 ; i < y_size ; i++)
	{
		for (j = 0 ; j < x_size ; j++)
		{
		    fpBMP[i * 1024 + j] = 0X00ff0000;  /* Write Red */                
		}
	}
}
